...
func parseSQLiteColumns(out string) []ColumnSchema {
	...
	for _, line := range lines {
		trimmed := strings.TrimSpace(line)
		// Skip empty, borders, and header lines. Header starts with │ 0 │ cid │
		if trimmed == "" || strings.HasPrefix(trimmed, "┌") || strings.HasPrefix(trimmed, "├") || strings.HasPrefix(trimmed, "└") || strings.Contains(trimmed, "name") || strings.Contains(trimmed, "cid") {
			continue
		}
