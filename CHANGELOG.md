...
### Fixed
- **Local Dev Stability**: Resolved `CLOUDFLARE_API_TOKEN` requirement in `aerostack dev` by forcing local-only mode and enabling interactive terminal input for wrangler.
- **RPC Reliability**: Fixed internal server errors in managed workers by implementing SDK RPC fallbacks and ensuring authenticated service discovery.
